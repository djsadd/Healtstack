...
def ssl_payment_request(request, pk, id):
    ...
    # Присваиваем transaction_id приёму
    appointment.transaction_id = transaction_id
    appointment.save()
    
    # Создаём платёж сразу как "оплаченный"
    payment = Payment.objects.create(
        ...
        status="paid",  # <--- важно
        transaction_date=str(timezone.now().date()),  # или .strftime(...) если нужно
        currency="BDT"
    )
    
    # Отправляем на страницу подтверждения
    return render(request, 'payment/success.html', {
        'payment': payment,
        'message': 'Оплата прошла автоматически (интеграция отключена)'
    })
